...
class Model extends \Kotchasan\KBase
{
    ...
    public static function send($id)
    {
        ...
        $order = \Kotchasan\Model::createQuery()
            ...
            ->where(array('R.id', $id))
            ->first('R.job_id', 'V.serial', 'V.equipment', 'R.job_description', 'R.create_date', 'U.username', 'U.name');
        if ($order) {
            $ret = array();
            // ข้อความ
            $content = array(
                '{LNG_Repair} '.$order->job_id,
                '{LNG_Serial/Registration number} '.$order->serial,
                '{LNG_Equipment} '.$order->equipment,
